...
remotes::install_github("dmirman/gazer") # to install webgazeR
...
setwd(here::here("data", "combined_data"))

write.csv(edat, file="combined_eye_data_mono.csv")
```
...
write.csv(emstr, file="combined_behavioral_data_mono.csv")
...
emstr <- read_csv(here::here("data", "combined_data", "combined_behavioral_data_mono.csv"))

edat <- read_csv(here::here("data", "combined_data", "combined_eye_data_mono.csv"))
...
When using the above function, separate data frames are produced for subjects and trials. These can be added to the behavioral dataframe.
...
Here no subjects had a threshold below 5. However, 15 trials did, and they were removed.
...
Let's go through the same steps we went through above, but now we will use the Gorilla provided AOI coordinates.
...
Here is where we are going to use our coordinate information from above. We use the `assign_aoi` that is adaopted from the `gazeR` package (Geller et al., 2020) to loop through our object `dat_colnames` and assign locations (i.e., TR, TL, BL, BR) to our normalized `x` and `y` coordinates. This function will label non-looks and off screen coordinates with NA. To make it easier to read we change the numerals assigned by the function to actual screen locations. We are going to input our dataframe we created with the Gorilla provided coordinates.
...
With the gorilla provided coordinates the effect seems to be a bit stronger!
...
#' Up-sample gaze coordinates (x, y)
#'
#' Increase the sampling frequency for x and y coordinates to 1000Hz.
#'
#' @section Output:
#'
#' Inserts additional rows into the data with missing gaze values for x and y coordinates.
#' Adds a column, `UpSampled` to identify whether the data has been up-sampled.
#'
#' @section Up-sample to 1000Hz:
#'
#' Up-sampling occurs by inserting rows with additional time points and filling
#' missing values for gaze coordinates. This function does not interpolate the
#' missing up-sampled values; that will be handled later in the pipeline (e.g.,
#' smoothing and interpolation).
#'
#' @param x Dataframe containing gaze data with columns `Gaze_Position.x` and `Gaze_Position.y`.
#' @export
#'
gaze_upsample <- function(x) {
  for (trial in unique(x$trial)) {
    # Subset data for the current trial
    x_trial <- dplyr::filter(x, trial == trial)
    
    # Create a time series for up-sampling with additional rows for missing times
    time_up <- data.frame(trial = trial,
                          time = min(x_trial$time):max(x_trial$time))  # Add 1000Hz timepoints
    
    # Merge with the original data to insert missing time points
    x <- merge(x, time_up, by = c("trial", "time"), all = TRUE)
  }
  
  # Ensure the correct column order for readability
  x <- dplyr::relocate(x, subject, .before = "trial")
  
  # Fill missing gaze values for x and y coordinates using the "down" method
  x <- tidyr::fill(x,
                   -tidyselect::any_of(c("x_pred_normalised", "y_pred_normalised")),
                   .direction = "down")
...
#' Linearly interpolates x, y coordinates in the data
#'
#' @param datafile A dataframe containing x, y coordinates and related columns.
#' @param x_col Name of the x-coordinate column (default: 'x_pred_normalized').
#' @param y_col Name of the y-coordinate column (default: 'y_pred_normalized').
#' @param subject Name of the column for subjects.
#' @param trial Name of the column for trials.
#' @param maxgap Maximum number of NAs to interpolate.
#' @param hz Recording frequency of eye tracker.
#' @export
#' @import zoo
#'
#' @return A dataframe with interpolated x, y coordinates.
#'
interpolate_gaze <- function(datafile, x_col = "x_pred_normalized", y_col = "y_pred_normalized", subject, trial, maxgap = Inf, hz = NA) {
  
  # Convert maxgap from milliseconds to number of samples
  if (maxgap != Inf) {
    maxgap <- round(maxgap / (1000 / hz))
  }
  
  message("Performing linear interpolation on x, y coordinates")
  
  # Interpolate x and y coordinates
  xy_interp <- datafile %>%
    dplyr::group_by({{subject}}, {{trial}}) %>%
    dplyr::mutate(
      # Linear interpolation for x coordinates
      interp_x = zoo::na.approx(.data[[x_col]], maxgap = maxgap, na.rm = FALSE, rule = 2),
      
      # Linear interpolation for y coordinates
      interp_y = zoo::na.approx(.data[[y_col]], maxgap = maxgap, na.rm = FALSE, rule = 2)
    ) %>%
    dplyr::ungroup()
...
gaze_interp <- interpolate_gaze(d, x_col = "x_pred_normalised", y_col = "y_pred_normalised", subject, trial, maxgap = Inf, hz = 1000)

gaze_interp <- gaze_interp %>% 
  distinct()
...
#' Smooth gaze coordinates using a 300-ms-long median filter
#'
#' @param datafile A dataframe containing gaze coordinates (`x` and `y` columns).
#' @param x_col Name of the x-coordinate column (default: 'x_pred_normalized').
#' @param y_col Name of the y-coordinate column (default: 'y_pred_normalized').
#' @param subject Name of the subject column.
#' @param trial Name of the trial column.
#' @param hz Recording frequency of the eye tracker in Hz (e.g., 1000 for 1000Hz).
#' @export
#'
#' @return A dataframe with smoothed x, y coordinates.
#'
smooth_gaze_median_filter <- function(datafile, x_col = "x_pred_normalized", y_col = "y_pred_normalized", subject, trial, hz = 1000) {
  # Calculate the window size for the 300-ms filter
  window_size <- round(300 / (1000 / hz))  # 300 ms in samples

  # Median filter function for a single vector
  median_filter <- function(vec, window_size) {
    stats::runmed(vec, k = window_size, endrule = "keep")
  }
  
  message("Applying 300-ms-long median filter to gaze coordinates")
  
  # Apply the median filter to the gaze coordinates
  smoothed_data <- datafile %>%
    dplyr::group_by({{subject}}, {{trial}}) %>%
    dplyr::mutate(
      smoothed_x = median_filter(.data[[x_col]], window_size),
      smoothed_y = median_filter(.data[[y_col]], window_size)
    ) %>%
    dplyr::ungroup()
...
```{r}
d <- smooth_gaze_median_filter(gaze_interp, x_col = "interp_x", y_col = "interp_y", subject, trial, hz = 1000)
```
